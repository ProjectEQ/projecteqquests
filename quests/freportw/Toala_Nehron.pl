...
sub EVENT_SAY{
	if($text =~ /Hail/i) {
		quest::say("I am Toala Nehron. You must be A another young warrior aspiring to join the ranks of the Steel Warriors. We welcome all who would try. When you [have time] there is ah friend of mine I would like you to check on.");
	}
	if($text =~ /opal/i) {
