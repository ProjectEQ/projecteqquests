...
sub EVENT_SAY {
	if(quest::istaskactivityactive(500222,7)) {
		if($text=~/Hail/i) {
			...
			if(defined($qglobals{halloween_ratter_complete}) && $qglobals{halloween_ratter_complete} < 7){
			...
			if(defined($qglobals{halloween_ratter_complete_pvp}) && $qglobals{halloween_ratter_complete_pvp} < 7 && $pcpvp == 1){
