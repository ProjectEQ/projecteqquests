...
sub EVENT_SAY {
  if($text=~/hail/i) {
    quest::say("Hi-oh there, $name. You startled me. I'm busy compiling notes for my compendium; it'll be a complete [history] or Rivervale for an entire century. Imagine that!");
  }
  if($text=~/history/i) {
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 12372 => 1, 12373 => 1, 13077 => 1, 14019 => 1)) { #Cat Skull Cap, Fractured Femur, Minotaur Horn, Bunch of Optic Nerves
    quest::say("Oh, hey there, $name. I'm still working on my history book. In fact, I just wrote down the part where we first met. Here let me read it for you!' Bill clears his throat, 'Ahem, so I was writing my history book when Dryssa comes up to me and says 'Hail.' And so I reply. . .' Bill continues to ramble on for many minutes. Finally, he gets to the end of his journal and looks up. He blinks as though surprised that you are still here. 'Oh, yeah, the armor pattern book. Sorry, I forgot about that. Here you go!");
