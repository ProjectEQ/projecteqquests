...
sub EVENT_SAY {
	if(quest::istaskactivityactive(500222,9)) {
		if($text=~/Hail/i) {
			...
			if(defined($qglobals{halloween_ratter_complete}) && $qglobals{halloween_ratter_complete} > 8){
			...
			if(defined($qglobals{halloween_ratter_complete_pvp}) && $qglobals{halloween_ratter_complete_pvp} > 8 && $pcpvp == 1){
