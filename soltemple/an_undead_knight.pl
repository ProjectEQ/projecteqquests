...
sub EVENT_ITEM {
  if (quest::handin({12292 => 1, 12291 => 1, 16507 => 1})) { # Items: Decayed Left Gauntlet, Decayed Right Gauntlet, Enchanted Platinum Bar
    quest::say("Well done, $name, here is your reward.");
    quest::summonitem(3144); # Item: Darkforge Gauntlets
  }
  elsif (quest::handin({12294 => 1, 12293 => 1, 12297 => 2})) { # Items: Decayed Left Legplate, Decayed Right Legplate, Melatite x 2
    quest::say("Well done, $name, here is your reward.");
    quest::summonitem(3145); # Item: Darkforge Greaves
  }
  elsif (quest::handin({12296 => 1, 12295 => 1, 12106 => 2})) { # Items: Decayed Left Boot, Decayed Right Boot, Fairy Dust x 2
    quest::say("Well done, $name, here is your reward.");
