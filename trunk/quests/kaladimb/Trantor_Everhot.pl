...
my $ItemCount = 0; #Tumpy Tonic Counter
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 12114 => 1)) { #Tumpy Tonic (Only 1)
    if($ItemCount == 0) {
  ...
  elsif(plugin::check_handin(\%itemcount, 12114 => 2)) { #Tumpy Tonic (Only 2)
    quest::say("Thank you, that hit the spot!! Here is the spool, as promised."); #Real text still needed
    quest::summonitem(12111); #Dwarven Wire
    quest::exp(1000);
    quest::faction(314,5); #Storm Guard
    ...
    quest::faction(57,-5); #Craknek Warriors
    if($ItemCount == 1) {
      quest::say('Hey!! Only one?!!'); #Visual effect only
    }
  }
  else {
