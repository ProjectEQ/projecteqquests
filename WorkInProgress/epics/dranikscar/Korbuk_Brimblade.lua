...
function event_say(e)
	...
	if(e.other:Class() == "Warrior") then
		if(e.message:findi("hail")) then
			...
			if(e.other:HasItem(60321)) then --Epic 1.5
				if(qglobals["warrior_epic"] < "16" or qglobals["warrior_epic"] == nil) then
		...
		elseif(e.message:findi("strange")) then
			e.self:Say("Aha, you have sensed it as well. For a while, I believed it was just me dat noticed it. Now dat you've confirmed my suspicions, I'm sure I can move forward with my [work].");
		elseif(e.message:findi("work")) then
			if(e.other:HasItem(17859) or e.other:HasItem(17869) or qglobals["warrior_pre"] == "6") then --Have old Red Scabbard or New Red Scabbard or have finished prequest
				...
				if(qglobals["warrior_epic"] < "1" or qglobals["warrior_epic"] == nil) then
					eq.set_global("warrior_epic","1",5,"F"); --flagged to start epic 1.5
				end
			else
				e.self:Say("Hmm, I'm not sure if I should go into any more detail. After all, we've just met and I don't know how trustworthy you really are. Listen, if you really want to know what work I'm involved with, I'll need you to run a couple [errands] for me first, just so I know you're da real deal.");
		...
		elseif(e.message:findi("glimpse") and qglobals["warrior_epic"] >= "10") then
			e.self:Say("It was a series of images, really. One was of a [young combatant] within da Muramite ranks. One was of a [gorge] somewhere. Da third was a [maze] of some sort, and da final one was in a [field of blood]. At da end, I saw four eyes dat appeared in da center of da hilt. I'm guessing da eyes go in those four holes on each side of da hilt and dat one each can be found in those four places.");
		...
		elseif(e.message:findi("another image") and qglobals["warrior_epic"] >= "14") then
			e.self:Say("This one is of da completed sword. It's not too shabby, but it's not great either. It looks like there will be more dat needs to be done than just what we have so far. In any case, I know how to get da sword put back together. It'll involve a trip to see my brother, [Krekk].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20426}) and qglobals["warrior_pre"] == "1") then --hand in Korbuk's Weapon Plans (dropped by The Diaku Overseer in PoTactics)
		e.self:Say("Ahh, da plans! You really gave it to dat overseer, huh? Not too bad, I must say. You're not done yet though. There are still a couple errands left dat I need finished. You know da most important part of any sword is da blade it's forged from, right? Well, it is. I had da perfect [shank] ready and waiting for me.");
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 60299}) and qglobals["warrior_epic"] >= "2") then --Recounted History of War (get from previous step)
		...
		e.self:Say("Things are getting stranger by da moment. I just had a vision of a creature dat wanders about da area below, then it went dark. I'm not sure if this creature is part of everything else, but it certainly is something we should check out. I know it seems silly, but you should have this [rock].");
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 60301}) and qglobals["warrior_epic"] >= "2") then --Blackfall Ore (From previous step)
		e.self:Say("And you say you found this in a pile dat da creature left behind? I wonder what da story behind dat is... In any case, touching this gave me a brief glimpse of something sharp. It almost seemed like it was a blade. You should probably take this to get it looked at by a skilled [smith].");
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 60303}) and qglobals["warrior_epic"] >= "2") then --Blackfall Blade (from previous step)
		e.self:Say("What a fine looking blade, I must say dat smith did good work in such a short time. Hopefully it wasn't too much touble for you. I... oh my, this can't be good. Da creature... it's [loose]!");
		e.other:SummonItem(60303); --Blackfall Blade back
		if(qglobals["warrior_epic"] < "3") then
			eq.set_global("warrior_epic","3",5,"F");
		end
	end
	if(item_lib.check_turn_in(e.trade, {item1 = 60303}) and qglobals["warrior_epic"] >= "3") then --Vial of Blackfall Blood (from Vxed encounter)
		e.self:Say("Dat was too close! You did a good job though and stopped dat beast before it could get too far on Norrath. We don't know what it might have been capable of otherwise. I've sealed da blood of da beast in a vial for you for da [journey]. I also got another [vision] when I took hold of da vial.");
		eq.delete_global("warrior_epic_vxed");
		e.other:SummonItem(60305); --Sealed Vial of Blackfall Blood
	end	
	if(item_lib.check_turn_in(e.trade, {item1 = 60305}) and qglobals["warrior_epic"] >= "4") then --Potion of Blackfall Spirit (from Kimber in EF)
		e.self:Emote("gags as he takes hold of the potion, then quickly returns it to you.");
		e.self:Say("I had no idea such a strong presence would be coming from this after Kimber worked with it. I hope he's ok. Now dat we've taken care of dat, we can focus on the last [vision] I had.");
		if(qglobals["warrior_epic"] < "5") then
			eq.set_global("warrior_epic","5",5,"F");
		end
		e.other:SummonItem(60305); --Potion of Blackfall Spirit
	end	
	if(item_lib.check_turn_in(e.trade, {item1 = 60307}) and qglobals["warrior_epic"] >= "5") then --Decrepit Hilt (from Girplan Devourer in Bloodfields)
		...
		if(qglobals["warrior_epic"] < "6") then
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 60312, item2 = 60313, item3 = 60314, item4 = 60315}) and qglobals["warrior_epic"] >= "10") then --4 diff Stone of Eternal Power
		...
		if(qglobals["warrior_epic"] < "11") then
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 60320}) and qglobals["warrior_epic"] >= "13") then --Hitl of Eternal Power
		e.self:Say("Met up with Gridbar did you? He's an oddball, for certain, but he's da best at what he does. I don't think I've ever seen stones set so finely into anything before, wouldn't you say? Alrighty, I think... wow [another image]...");
		e.other:SummonItem(60320); --Hilt of Eternal Power
		if(qglobals["warrior_epic"] < "14") then
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 47100}) and qglobals["warrior_epic"] == "18") then --Globe of Discordant Energy
		e.self:Say("Yes, this is precisely what I felt. Its power is indescribable. We're so close now! I can see what's left and it's a glorious end to a glorious journey. There are just two [last items] you need to take care of.");
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 60321, item2 = 60327}) and qglobals["warrior_epic"] == "19") then --Kreljnok's Focused Rage and Champion's Sword of Eternal Power
		e.self:Say("Well done. Well done, indeed. I knew da moment I saw you dat you were da right person for all of this. Here is da sword, but there's one [last item] dat you need to do.");
