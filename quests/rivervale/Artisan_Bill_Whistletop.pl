...
sub EVENT_SAY {
  if($race eq "Halfling") {
    ...
    if(($text=~/the hole/i) && ($ulevel >= 40)) {
      quest::taskselector(204);
    }
    else {
      quest::say("I'm sorry, I don't think you are ready for that task yet. Please come back once you've gotten a bit more experience.");
    }
    if(($text=~/karnor/i) && ($ulevel >= 60)) {
      quest::taskselector(205);
    }
    else {
      quest::say("I'm sorry, I don't think you are ready for that task yet. Please come back once you've gotten a bit more experience.");
    }
    if(($text=~/stillmoon/i) && ($ulevel >= 70)) {
...
sub EVENT_ITEM {
  if($race eq "Halfling") {
    if(plugin::check_handin(\%itemcount, 12372 => 1, 12373 => 1, 13077 => 1, 14019 => 1)) { #Cat Skull Cap, Fractured Femur, Minotaur Horn, Bunch of Optic Nerves
      quest::say("Oh, hey there, $name. I'm still working on my history book. In fact, I just wrote down the part where we first met. Here let me read it for you!' Bill clears his throat, 'Ahem, so I was writing my history book when Dryssa comes up to me and says 'Hail.' And so I reply. . .");
      quest::emote("continues to ramble on for many minutes. Finally, he gets to the end of his journal and looks up. He blinks as though surprised that you are still here. 'Oh, yeah, the armor pattern book. Sorry, I forgot about that. Here you go!'");
