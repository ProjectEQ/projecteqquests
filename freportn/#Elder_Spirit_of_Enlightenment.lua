...
function event_trade(e)
	...
	if(qglobals["shaman_epic"] == "8" and item_lib.check_turn_in(e.trade, {item1 = 57551})) then --hand in Impervious Medicine Bag (received from Lupot Nukla in Fungus Grove)
	...
	if(qglobals["shaman_epic"] == "9" and item_lib.check_turn_in(e.trade, {item1 = 57987})) then --hand in Glowing Spirit Charm of Might (made from combine, probably needs added to db)
	...
	if(qglobals["shaman_epic"] == "10" and item_lib.check_turn_in(e.trade, {item1 = 57404})) then --hand in Spiritcaller Beads (looted from Discordling Spiritcaller in Walls of Slaughter)
	...
	if(qglobals["shaman_epic"] == "11" and item_lib.check_turn_in(e.trade, {item1 = 47100})) then --hand in Globe of Discordant Energy (from Citadel of Anguish)
		e.self:Emote("lowers its head and you believe you see a shade of a tear drop from its eyes and disappear into nothingness.");
		e.self:Say("You have done it. You cannot imagine our gratitude. The time for Ruchu is at hand and we offer you the privilege of taking part. If you are able to do the ceremony now, tell me you are [" .. eq.say_link("prepared for the ceremony") .. "].");
		eq.set_global("shaman_epic","12",5,"F");
	end
	if(qglobals["shaman_epic"] == "12" and item_lib.check_turn_in(e.trade, {item1 = 57400})) then --hand in Crafted Talisman of Fates (Epic 1.5)
