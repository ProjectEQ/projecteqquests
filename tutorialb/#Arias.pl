...
sub EVENT_SAY {
  if ($text=~/hail/i && quest::istaskactivityactive(1448,0)){
	...
	quest::updatetaskactivity(1448,0); 
}
  elsif ($text=~/hail/i && quest::istaskactivityactive(1448,7)){
    ...
    quest::updatetaskactivity(1448,7); 
}
  elsif ($text=~/hail/i){
    quest::say("We found the other slaves! Not bad, my friend, not bad. No matter what happens in the mines, you should always be able to find your way back here.");
}
