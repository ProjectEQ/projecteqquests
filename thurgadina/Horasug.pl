sub EVENT_WAYPOINT_ARRIVE {
	if($wp==1) {
		...
		quest::signalwith(115011,1); # NPC: Rumagur
		quest::signalwith(115187,1); # NPC: Rumagur
	}
	if($wp==2) {
		quest::say("Uh oh");
		quest::SetRunning(1);
		quest::signalwith(115011,2); # NPC: Rumagur
		quest::signalwith(115187,2); # NPC: Rumagur
		quest::signalwith(115010,3); # NPC: Saramor
		quest::signalwith(115012,2); # NPC: Jorumin
		quest::signalwith(115188,2); # NPC: Jorumin
	}
	if($wp==19) {
		quest::SetRunning(0);
		quest::say("Whew, that was a close one!");
		quest::signalwith(115142,1); # NPC: Doogle_McBanick
	}
	if($wp==42) {
		$npc->SetAppearance(1);
	}
...
sub EVENT_SIGNAL {
	if($signal==1) {
		...
		quest::signalwith(115010,1); # NPC: Saramor
		quest::signalwith(115014,1); # NPC: Loremaster_Fronden
	}
	if($signal==2) {
		...
		quest::signalwith(115010,2); # NPC: Saramor
		quest::signalwith(115012,1); # NPC: Jorumin
		quest::signalwith(115188,1); # NPC: Jorumin
	}
	if($signal==3) {
		quest::say("I had a sacred vision from Brell himself!");
		quest::signalwith(115142,2); # NPC: Doogle_McBanick
	}
	if($signal==4) {
		quest::emote("thinks for a moment, looking at the roomful of rugged patrons,");
		quest::signalwith(115142,3); # NPC: Doogle_McBanick
	}
	if($signal==5) {
		quest::say("Heh, I saw meself here at Doogie's Drinks partaking of the finest Coldain ale in the land, of course!");
		quest::signalwith(115142,4); # NPC: Doogle_McBanick
	}
}