...
function event_say(e)
	...
	elseif (e.message:findi("gnome") and e.other:Class() == "Wizard") then
...
function event_trade(e)
	...
	if (eq.handin({[14335] = 1})) then --Item: Arantir's Ring -- Return Love Note handin
		eq.set_global("wizepicA","1",1,"F");
	elseif (eq.handin({[18168] = 1})) then --Item: Note to Arantir -- Return Oil Note handin
		eq.delete_global("wizepicA");
		eq.set_global("wizepicB","1",1,"F");
	elseif (qglobals["wizepicB"] ~= nil) then
		if (eq.handin({[14337] = 1, [14338] = 1, [14339] = 1})) then --Items: Staff of Gabstik, Blue Crystal Staff, Gnarled Staff
