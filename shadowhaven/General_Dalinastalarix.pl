sub EVENT_WAYPOINT_ARRIVE {
	...
	quest::signalwith(150269,1,5); # NPC: Ronlotar_Stumpshade Boot Merchant
	}
	if ($wp == 6) {
	...
	quest::signalwith(150269,2,5); # NPC: Ronlotar_Stumpshade Boot Merchant
	}
	if ($wp == 10) {
	quest::say("Afternoon Lookout Gereol, everything is going good on this post as far as I can see.");
	quest::signal(150049,5); # NPC: Lookout_Gereol
	}
	if ($wp == 17) {
	...
	quest::signal(150036, 5); # NPC: Lookout_Valobolo
	}
	if ($wp == 18) {
	...
	quest::signal(150034,5); # NPC: Lookout_Falatalix
	quest::signal(150040,5); # NPC: Lookout_Reloen
	}
	if ($wp == 31) {
	...
	quest::signal(150021,5); # NPC: Lookout_Yereil
	quest::signal(150043,5); # NPC: Lookout_Uhingil
	}
}
