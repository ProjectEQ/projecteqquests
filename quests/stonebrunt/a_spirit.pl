sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::emote("flares brightly as you attempt to speak with it. By listening very closely you're brely able to make out it's frantic whispering. It keep refering to one of the Kejekans, one that apparently slew him as
he attempted to cleanse the land of them. It wants nothing more than to destroy who killed him, but it cannot in this [form]. ");
  }
  if ($text=~/form/i) {
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 2100 =>1 )) {
    ...
    quest::exp(10000);
    quest::summonitem(2297);
    quest::depop();
    quest::spawn2(100221,0,0,-70,474,509,33.5);
