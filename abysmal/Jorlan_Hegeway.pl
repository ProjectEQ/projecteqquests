...
sub EVENT_SAY {
  ...
  if ($text=~/unique chemicals/i) {
    ...
    quest::taskselector(500171); #Alchemy Collect
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58074 => 1)) { #Sample of Filtered Taelosian Sludge
    if (quest::istaskactivityactive(500171,0)) { #Alchemy Collect Step 1
    ...
    elsif (quest::istaskactivityactive(500171,1)) { #Alchemy Collect Step 2
    ...
    elsif (quest::istaskactivityactive(500171,2)) { #Alchemy Collect Step 3
    ...
    elsif (quest::istaskactivityactive(500171,3)) { #Alchemy Collect Step 4
