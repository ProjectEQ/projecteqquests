...
function event_say(e)
	...
	if(e.other:Race() ==  "Froglok" or fac < 5) then
		if(e.message:findi("hail")) then
			e.self:Say("Salutations. Come sit with me and have a drink. I am just going over some [" .. eq.say_link("business") .. "] right now.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 55035})) then -- Sir Bronthas's Remains
		if(e.other:Race() ==  "Froglok" or fac < 5) then
			...
			e.other:SummonItem(55036); -- Marr's Blessing
			e.other:Ding();
			...
			e.other:AddEXP(1750000); -- ~1% of exp for level 51
		else
			e.self:Say("Hmm... there is something about " .. e.other:Race() .. "s that I don't exactly trust...");
			e.other:SummonItem(55035); -- Sir Bronthas's Remains
		end
	end
