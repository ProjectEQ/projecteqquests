...
sub EVENT_SAY {
  if(plugin::check_hasitem($client,15784)) { #Orb of Deathspeaking
    ...
    if(($text=~/i want the staff/i) && ($Soulwhisper==1)) {
      ...
      quest::setglobal("Soulwhisper",2,0,"F");
    }
    if(($text=~/power/i) && ($Soulwhisper==3)) {
      ...
      quest::setglobal("Soulwhisper",4,0,"F");
    }
...
sub EVENT_ITEM {
  if((plugin::check_handin(\%itemcount, 11174 => 1)) && ($Soulwhisper==2)) { #Whiahdi's Earthly Possessions
  ...
  elsif((plugin::check_handin(\%itemcount, 55855 => 1)) && ($Soulwhisper==3)) { #Tarlang's Head
...
#EndFile: natimbi\#Whiahdi.pl (00000)