...
sub EVENT_SIGNAL {
	...
	if($signal == 3) {
		...
		quest::signalwith(160112,1,2500); # NPC: Arbogast
	}
	if($signal == 4) {
	...
	if($signal == 5) {
		...
		quest::signalwith(160112,2,2500); # NPC: Arbogast
	}
	if($signal == 6) {
		plugin::DoAnim("cheer");
		quest::signalwith(160112,4,500); # NPC: Arbogast
	}
	if($signal == 7) {
		quest::start(24);
	}
	if($signal == 8) {
		...
		quest::signalwith(160112,5,2500); # NPC: Arbogast
	}
	if($signal == 9) {
		quest::say("Wait for me!");
		quest::spawn2(160376,26,0,$x,$y,$z,$h); # NPC: #Miller
