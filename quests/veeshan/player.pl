sub EVENT_CLICKDOOR {
   ...
   if(($doorid == 56 || $doorid == 57 || $doorid == 312 || $doorid == 
313) && ($dragonsup == 0)){
   $client->Message(0,"You got the door open.");
   quest::forcedooropen(56);
   quest::forcedooropen(57);
   }
  elsif($doorid == 56 || $doorid == 57 || $doorid == 312 || $doorid == 
313) {
  $client->Message(0,"A seal has been placed on this door by Phara Dar. 
Perhaps there is a way to remove it.");
 }  
}
