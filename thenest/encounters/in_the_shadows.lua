...
function Vish_Death(e)
	...
	if eq.get_data("DoN_Vish_First") == "" then
		local target = e.other;
		if e.other:IsPet() then
			target = e.other:GetOwner();
		end

		eq.world_emote(MT.Magenta,"Congratulations to [".. eq.get_guild_name_by_id(eq.get_guild_id_by_char_id(target:CastToClient():CharacterID())) .."] for being the first guild to purge the land of Vishimtar the Fallen and completing the Dragons of Norrath Expansion.");
		eq.set_data("DoN_Vish_First", "1");
	else
		e.self:DeathNotification(e);

		local data_bucket = ("DoN_Vish_Kill_Timer");
		local dz = target:CastToClient():GetExpedition();
		local dz_time = eq.seconds("6h");
		local remaining_time = eq.task_time_left(eq.get_dz_task_id());
		...
		eq.world_emote(MT.Magenta,"Congratulations to [".. eq.get_guild_name_by_id(guild_id) .."] for killing Vishimtar the Fallen and purging the corruption from Norrath. Total Time from Zone Start: ".. eq.SecondsToClockHours(total_time_seconds));

		if record_kill_exists then
			eq.zone_emote(MT.Lime, "Vishimtar: Current Zone Speed Record: Guild = ".. eq.get_guild_name_by_id(record_kill_guild_id) .." -  Time To Complete = ".. eq.SecondsToClockHours(record_kill_seconds));
		end

		if record_kill then
			eq.zone_emote(MT.LightBlue, "This run was a new record! Congratulations!");
		end
	end
end
