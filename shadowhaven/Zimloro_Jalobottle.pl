...
sub EVENT_ITEM { 
  if (plugin::check_handin(\%itemcount, 4759 => 1)) { #Item: Antidote Ingredients
    ...
    quest::faction(1512, 5); #Faction: House of Stout
    quest::faction(1508, 1); #Faction: Traders of the Haven
    quest::faction(500);
  }
  if (plugin::check_handin(\%itemcount, 4765 => 1)) { #Item: Antidote Supplies
