...
function event_click_door(e)
  ...
  if (door_id == 2) then 
	...
	if (inst_id > 0 ) then
		e.self:MovePCInstance(318, inst_id, 3522.5, 1584.2, 2.8, 140); -- Zone: roost
    else		
  ...
  elseif (door_id == 3) then  
	...
	if (inst_id > 0 ) then
		e.self:MovePCInstance(319, inst_id, 0, -447 , -38.5, 508); -- Zone: ruja
    else		
      e.self:Message(13, "You are not part of an instance.");		
	end
	--murkglider
  elseif (door_id==4) then
	...
	if (inst_id > 0 ) then
		e.self:MovePCInstance(320, inst_id, 3944, -252, 139, 508); -- Zone: rujb
    else		
      e.self:Message(13, "You are not part of an instance.");		
	end
  end
