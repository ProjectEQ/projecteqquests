sub EVENT_SAY {
  ...
  if ($text=~/hail/i) {
    ...
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 10)) { 
      quest::say("I wasn't expecting you back so soon, $name, but I'm glad you're here. I've been analyzing the glyphs on the last three artifacts you recovered and, the way I read them, it seems the trusik priests used the [four temples] around the Altar of Destruction to gain access to a temple to face some kind of final rite passage.");
    }
    if (defined($qglobals{ikkypit}) && ($qglobals{ikkypit} == 1)) { 
      quest::say("Welcome back, $name. I've been looking over those artifacts you returned to me in the sealed bag and I'm afraid I have some disturbing news. It seems the artifacts weren't artifacts at all, but objects of ghastly power held by the dark spirits you collected them from. What's worse is that since you've collocted these objects, the energy emanating from the pit has [gotten stronger]!");
    }
	if (defined($qglobals{ikky}) && ($qglobals{ikky} >= 2)) {
	  $client->Message(15, "Tublik Narwethar says, 'If you want to see what you've completed at any time, just ask me for a [progress update]!'");
	}
    else {
  ...
  if (defined($qglobals{ikky}) && ($qglobals{ikky} >= 10)) {
    ...
    if (plugin::check_handin(\%itemcount, 60170 => 1)) {
      if (!defined($qglobals{ikkyraid})) {
	    quest::say("You've done it! You've recovered the first of the three artifacts! You must keep this with you, as you'll need it once you've recovered all three artifacts. You only need two more -- one from [Sanctuary of the Glorified], and one from the [Sanctuary of the Transcendent]. Which do you think you'd like to do next?");
    ...
    if (plugin::check_handin(\%itemcount, 60171 => 1)) {
      if (defined($qglobals{ikkyraid}) && ($qglobals{ikkyraid} == 1)) {
	    quest::say("You've done it! You've recovered the second of the three artifacts! You must keep this with you, as you'll need it once you've recovered all three artifacts. You only need one more from [Sanctuary of the Transcendent]. Are you ready to progress forward with the final artifact?");
