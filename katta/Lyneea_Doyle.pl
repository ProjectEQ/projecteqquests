...
sub EVENT_TIMER {
	...
	quest::signalwith(160223,1,8000); # NPC: Olam_Polaja
	quest::stoptimer(1);
}
...
sub EVENT_SIGNAL {
	if($signal == 1) {
		...
		quest::signalwith(160223,2,8000); # NPC: Olam_Polaja
	}
	if($signal == 2) {
		quest::emote("attempts to flip a copper into a mug on the table and succeeds!");
		quest::signalwith(160223,3,8000); # NPC: Olam_Polaja
	}
	if($signal == 3) {
		quest::signalwith(160223,4,8000); # NPC: Olam_Polaja
	}
	if($signal == 4) {
		quest::emote("drinks a gulp of ale");
		quest::signalwith(160223,5,1); # NPC: Olam_Polaja
	}
	if($signal == 5) {
		quest::emote("attempts to flip a copper into a mug on the table and succeeds!");
		quest::signalwith(160223,6,8000); # NPC: Olam_Polaja
	}
	if($signal == 6) {
		quest::signalwith(160223,7,8000); # NPC: Olam_Polaja
	}
	if($signal == 7) {
		quest::emote("attempts to flip a copper into a mug on the table and succeeds!");
		quest::signalwith(160223,8,8000); # NPC: Olam_Polaja
	}
	if($signal == 8) {
		quest::say("Are you feeling all right?");
		quest::signalwith(160223,9,8000); # NPC: Olam_Polaja
	}
	if($signal == 9) {
