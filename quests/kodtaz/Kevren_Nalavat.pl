sub EVENT_SAY {
  if ($text=~/hail/i) {
	...
	if (defined($qglobals{ikky}) && ($qglobals{ikky} >= 2)) {
	  $client->Message(15, "Kevren Nalavat says, 'If you want to see what you've completed at any time, just ask me for a [progress update]!'");
	}
    else {
  ...
  if ($text=~/trials/i) {
    quest::emote("look turns serious. 'There are any number of new threats with these invaders and we're unsure just how powerful or how many of them there are. We need to make sure that those of you being sent up here to find that information out for us won't be scared off by the destructive force these invaders bring along with them. The trials must be done in a [specific order] and must all be completed before we can allow you to attempt any further work for the brotherhood.'");
  }
  ...
  if (($text=~/ancient relics/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    quest::say("We're unsure what the invaders are gathering these particular relics for. From what I can surmise, none of them have any kind of magical properties. In any case, it's up to you and a group of adventurers to venture to the Martyrs Passage and [investigate] the situation. I've heard reports of ghosts in that area, so the invaders may have stirred up something unexpected.");
  }
  if (($text=~/investigate/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    quest::say("You'll need to head to the Martyrs Passage and recover four different relics. I advise taking a sturdy group with you because there may be invaders there in the process of burgling the passage. If you uncover any information on what the Mata Muram are planning to use these relics for, return that to me as well, Do you [understand] your objective?");
  }
