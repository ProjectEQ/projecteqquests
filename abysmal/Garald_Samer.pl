...
sub EVENT_SAY {
  ...
  if ($text=~/work/i) {
    ...
    quest::taskselector(500176); #Tailoring Collect
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58053 => 1)) { #Preserved Muramite Etched Scales
    if (quest::istaskactivityactive(500176,0)) { #Tailoring Collect Step 1
    ...
    elsif (quest::istaskactivityactive(500176,1)) { #Tailoring Collect Step 2
    ...
    elsif (quest::istaskactivityactive(500176,2)) { #Tailoring Collect Step 3
    ...
    elsif (quest::istaskactivityactive(500176,3)) { #Tailoring Collect Step 4
