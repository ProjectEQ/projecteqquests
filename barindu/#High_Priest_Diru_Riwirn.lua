...
local sewers = {
  { -- should be version 0 of zone, named can spawn in progression version
...
for i=1,#sewers do
  sewers[i].dz_info.compass = { "barindu", -645.859, -338.502, -118.309 }
  sewers[i].dz_info.safereturn = { "barindu", -605.0, -337.0, -123.84, 0.0 }
end

local function create_sewer_expedition(client, trial)
  local dz = client:CreateExpedition(trial.dz_info, trial.expedition_info)
  if dz.valid then
...
function event_say(e)
  ...
  if e.message:findi("hail") then
    ...
    elseif sewers_flag == 1 and not snplant_complete then
      ...
      create_sewer_expedition(e.other, sewers[1])
    ...
    elseif sewers_flag == 2 and not sncrematory_complete then
      ...
      create_sewer_expedition(e.other, sewers[2])
    ...
    elseif sewers_flag == 3 and not snlair_complete then
      ...
      create_sewer_expedition(e.other, sewers[3])
    ...
    elseif sewers_flag == 4 and not snpool_complete then
      ...
      create_sewer_expedition(e.other, sewers[4])
