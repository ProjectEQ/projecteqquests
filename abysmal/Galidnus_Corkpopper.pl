...
sub EVENT_SAY {
  ...
  if ($text=~/some work/i) {
    ...
    quest::taskselector(500168); #Brewing Collect
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58152 => 1)) { #Taelosian Tea
    if (quest::istaskactivityactive(500168,0)) { #Brewing Collect Step 1
    ...
    elsif (quest::istaskactivityactive(500168,1)) { #Brewing Collect Step 2
    ...
    elsif (quest::istaskactivityactive(500168,2)) { #Brewing Collect Step 3
    ...
    elsif (quest::istaskactivityactive(500168,3)) { #Brewing Collect Step 4
