...
function event_signal(e)
	...
	if (e.signal == 0) then
		...
		if ( eq.get_entity_list():IsMobSpawnedByNpcTypeID(293213) == false and eq.get_entity_list():IsMobSpawnedByNpcTypeID(293120) == false ) then
			...
			eq.spawn2(293212, 0, 0, 591, 746, -460, 150);

			-- Spawn Deranged Lesser Stoneservants
			eq.spawn2(293215,0,0,611,683,-460,64);
			eq.spawn2(293215,0,0,589,771,-459,176);
			eq.spawn2(293215,0,0,673,821,-460,266);
			eq.spawn2(293215,0,0,750,754,-460,360);
			eq.spawn2(293215,0,0,705,668,-453,468);

			-- Spawn Deranged Greater Stoneservants
			eq.spawn2(293216,0,0,581,723,-460,118);
			eq.spawn2(293216,0,0,617,806,-449,206);
			eq.spawn2(293216,0,0,720,803,-460,300);
			eq.spawn2(293216,0,0,744,707,-460,414);
			eq.spawn2(293216,0,0,644,654,-460,14);
		end
	elseif (e.signal == 1) then
		-- Signal from Grand Summoner saying he died
		-- Spawn the Deranged Supreme Stoneservants
		eq.spawn2(293217,0,0,611,683,-460,64);
		eq.spawn2(293217,0,0,589,771,-459,176);
		eq.spawn2(293217,0,0,673,821,-460,266);
		eq.spawn2(293217,0,0,750,754,-460,360);
		eq.spawn2(293217,0,0,705,668,-453,468);
		eq.depop_with_timer();
	...
	elseif (e.signal == 2) then
		...
		if ( eq.get_entity_list():IsMobSpawnedByNpcTypeID(293215) == false ) then
			eq.spawn2(293214,0,0,684,719,-460,450);   
		end
	...
	elseif (e.signal == 3) then
		...
		eq.depop(293212);
		eq.spawn2(293113,0,0,591,746,-460,150);
...
function respawn_event(e)
	...
	eq.spawn2(293213, 0, 0, 613, 789, -450, 188);
	eq.spawn2(293213, 0, 0, 692, 810, -456, 344);
	eq.spawn2(293213, 0, 0, 725, 770, -460, 324);
	eq.spawn2(293213, 0, 0, 717, 690, -449, 480);
	eq.spawn2(293213, 0, 0, 596, 703, -456, 88);

	-- Spawn 293120 ##Pixtt Priest Summoner
	eq.spawn2(293120, 0, 0, 651, 815, -460, 226);
	eq.spawn2(293120, 0, 0, 740, 723, -456, 440);
	eq.spawn2(293120, 0, 0, 677, 669, -451, 488);
	eq.spawn2(293120, 0, 0, 623, 672, -460, 28);

	-- Spawn 293211 Pixtt High Summoner
	eq.spawn2(293211, 0, 0, 591, 746, -460, 150);
end
