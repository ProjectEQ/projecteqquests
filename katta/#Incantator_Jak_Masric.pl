...
sub EVENT_SIGNAL{
	if($signal == 1){
		...
		quest::signalwith(160458,2,5000); # NPC: Autarkic_Lord_Sfarosh
	}
	if($signal == 2){
		quest::say("The circle of containment is breaking down without Ajeea! Prepare yourselves!");
		quest::signalwith(160458,5,5000); # NPC: Autarkic_Lord_Sfarosh
	}
	if($signal == 3){
		...
		quest::signalwith(160458,6,5000); # NPC: Autarkic_Lord_Sfarosh
		quest::depop();
	}
}
