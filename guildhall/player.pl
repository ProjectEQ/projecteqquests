...
sub EVENT_POPUPRESPONSE {
  ...
  if ($popupid == 2708) { #northkarana
    quest::movepc(13,1209,-3685,-5); # Zone: atiiki
  }
  if ($popupid == 4176) { #ecommons
    quest::movepc(22,-140,-1520,3); # Zone: bertoxtemple
  }
  if ($popupid == 534) { #lavastorm
    quest::movepc(27,460,460,-86); # Zone: bothunder
  }
  if ($popupid == 2707) { #tox
    quest::movepc(38,-916,-1510,-33); # Zone: chambersb
  }
  ...
  if ($popupid == 2031) { #coboltscar
    quest::movepc(117,-1634,-1065,299); # Zone: ferubi
  }
  if ($popupid == 3615) { #twilight
    quest::movepc(170,-1028,1338,39); # Zone: hohonora
  }
  if ($popupid == 3794) { #stonebrunt
    quest::movepc(100,673,-4531,0); # Zone: eastkorlach
  }
  if ($popupid == 6180) { #wallofslaughter
    quest::movepc(300,-943,13,130); # Zone: qcat
  }
  if ($popupid == 5733) { #barindu
    quest::movepc(283,209,-515,-119); # Zone: poearthb
  }
  ...
  if ($popupid == 20543) { #potimea
    quest::movepc(219,0,110,8); # Zone: mirg
  }
...
#      if ($client->CalculateDistance(-3.4, -162.9, 6.9) <= 30) {
        quest::MovePCInstance(344, 5, 0, 588, 1.75, 260); # Zone: sirens
#      }
    }
  }
}
