...
sub EVENT_ITEM { 
  ...
  # White Wine ID-13031
  my $wine = 0;
  if (plugin::check_handin(\%itemcount,13031=>4)) {$wine=4;
  } elsif (plugin::check_handin(\%itemcount,13031=>3)) {$wine=3;
  } elsif (plugin::check_handin(\%itemcount,13031=>2)) {$wine=2;
  } elsif (plugin::check_handin(\%itemcount,13031=>1)) {$wine=1;
  }
  if ($wine>=1) {
    for ($i=0; $i<$wine; $i++) {
      ...
      quest::ding();
      quest::exp(25);
      quest::faction(99,2); # Faction Faydark's Champions
      quest::faction(178,2); # Faction King Tearis Thex
      quest::faction(34,2); # Faction Clerics of Tunare
      quest::faction(304,2); # Faction Soldiers of Tunare
	  quest::faction(100,2); #Faction Felguard - Only NPC in the game with it.
      quest::faction(63,-4); # Faction Crushbone Orcs 
    }
  }
