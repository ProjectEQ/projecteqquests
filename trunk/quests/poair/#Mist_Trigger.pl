...
sub EVENT_SIGNAL {
     if ($signal == 1 && defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) {
        $counter+=1;
     	if ($signal == 1 && $counter == 4 && defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) {
			quest::spawn2(215070,0,0,323.7,-721.3,439.3,82.9);
        }
     }
     if ($signal == 2 && defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) {
        quest::spawn2(215071,0,0,-1571.6,-570.0,343.6,190.9);
		quest::depop(215023);
     }
     if ($signal == 3  && !defined($qglobals{mist_done})) {
        quest::setglobal("mist_done",1,3,"D2");
     }
	 if ($signal == 4 && !defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) {
		...
		if($check == 0 && !defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) {
