...
sub EVENT_SAY {
	if(quest::istaskactivityactive(500222,5)) {
		if($text=~/Hail/i) {
			...
			quest::updatetaskactivity(500222,5);
			...
			if(defined($qglobals{halloween_ratter_complete}) && $qglobals{halloween_ratter_complete} > 4){
			...
			if(defined($qglobals{halloween_ratter_complete_pvp}) && $qglobals{halloween_ratter_complete_pvp} > 4 && $pcpvp == 1){
