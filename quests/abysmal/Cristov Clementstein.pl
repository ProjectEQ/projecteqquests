...
if($text=~/hail/i && $chrisredwinea == 1){
...
if (plugin::check_handin(\%itemcount, 55584 => 1)) { # Grape crushing device
...
if (plugin::check_handin(\%itemcount, 55580 => 1 && $chrisredwinea == 2)) { # Cristov's Finest Red Wine
...
#END of FILE zone:abysmal ID:279256 -- Cristov Clementstein.pl
