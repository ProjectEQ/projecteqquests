	...
	if(e.other:Class() == "Shadowknight") then
		if(e.message:findi("hail")) then
			...
			if(qglobals["shadowknight_epic"] == "1") then
				e.self:Say("You. You are the one I have seen in my visions.' Sienn removes a dark blade from a scabbard. I have just obtained this sword from one of my apprentices. This blade may be the key to the very existence of the shadowknight. Are you familiar with what has [been transpiring]?");
			end
...
function evetn_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20520})) then --hand in The Silent Gods (received from Grand Librarian Maelin)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20426}) and qglobals["shadowknight_epic"] == "2") then --hand in Sheath of Darktidings (result of a combine, probably needs added to db)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 23492}) and qglobals["shadowknight_epic"] == "3") then --hand in Fraga's Parchment (drops from Fraga in PoValor)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20497, item2 = 19025}) and qglobals["shadowknight_epic"] == "4") then --hand in Wailing Sister Blood and Wailing Rune (both drop from The Wailing Sister in the Plane of Nightmare)
