...
sub EVENT_SAY {
  ...
  if ($text=~/work/i) {
    ...
    quest::taskselector(500177); #Tinkering Collect
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58242 => 1)) { #Dark Matter
    if (quest::istaskactivityactive(500177,0)) { #Tinkering Collect Step 1
    ...
    elsif (quest::istaskactivityactive(500177,1)) { #Tinkering Collect Step 2
    ...
    elsif (quest::istaskactivityactive(500177,2)) { #Tinkering Collect Step 3
    ...
    elsif (quest::istaskactivityactive(500177,3)) { #Tinkering Collect Step 4
