...
function Priest_Death(e)
    if (eq.get_entity_list():IsMobSpawnedByNpcTypeID(293123) == false and eq.get_entity_list():IsMobSpawnedByNpcTypeID(293122) == true) then
    --if all 9 priest summoners dead and grand summoner alive, start next phase
        eq.signal(293122,1); --signal to start next phase
    end
end
