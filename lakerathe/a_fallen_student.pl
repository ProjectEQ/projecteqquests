...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    if ($client->GetGlobal("monk_epic") == undef) {
      ...
      if (plugin::check_hasitem($client, 10652) || $client->GetGlobal("MnkPre") == 3) {
  ...
  if ($text=~/task/i) { # Monk Epic 1.5/2.0 (Prequest)
  ...
  if ($text=~/the way/i && $client->GetGlobal("MnkPre") == 2) { # Monk Epic 1.5/2.0 (Prequest)
  ...
  if ($client->GetGlobal("monk_epic") == 1 && $text=~/vision/i) { # Monk Epic 1.5
  ...
  if ($client->GetGlobal("monk_epic") == 2 && $text=~/time/i) { # Monk Epic 1.5
  ...
  #if ($client->GetGlobal("monk_epic") == 2 && $text=~/power of the Immortals/i) { # Monk Epic 1.5
  ...
  if ($client->GetGlobal("monk_epic") == 3 && $text=~/Vortix/i) {
    ...
    quest::unique_spawn(203433,0,0,$x,$y,$z,$h); # NPC: Vortix
    quest::depop_withtimer();
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 48122 => 1)) { # Handin: Muramite Sand
    ...
    quest::setglobal("monk_epic", "2", 5, "F"); # Monk Epic 1.5
  }
  elsif (plugin::check_handin(\%itemcount, 48100 => 1, 48101 => 1)) { # Handin: Seal of Cedrick & Seal of Mystik
    ...
    quest::setglobal("MnkPre", "2", 5, "F"); # Monk Epic 1.5/2.0 (Prequest)
  }
  elsif(plugin::check_handin(\%itemcount, 48102 => 1)) { # Handin: Medal of Identity
    ...
    quest::setglobal("MnkPre", "3", 5, "F"); # Monk Epic 1.5/2.0 (Prequest)(END)
  }
